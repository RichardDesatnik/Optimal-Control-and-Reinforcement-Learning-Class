#include <Wire.h>
#include "SparkFun_Displacement_Sensor_Arduino_Library.h" 
ADS myFlexSensor;

int ActTime = 1000;
int ETime = 1000;
float command = -100;
int Finger1_1 = 2;
int Finger1_2 = 3;
int Finger1_3 = 4;
int Finger1_4 = 5;
int Finger2_1 = 6;
int Finger2_2 = 7;
int Finger2_3 = 8;
int Finger2_4 = 9;
int Finger3_1 = 10;
int Finger3_2 = 11;
int Finger3_3 = 12;
int Finger3_4 = 44;
int TimeA = 500;
int Active = 50;

void setup() {
  // put your setup code here, to run once:
Serial.begin(115200);
while (!Serial)
    ;
  Serial.println("SparkFun Displacement Sensor Example");

  Wire.begin();

  if (myFlexSensor.begin() == false)
  {
    Serial.println(F("No sensor detected. Check wiring. Freezing..."));
    while (1)
      ;
  }


pinMode(Finger1_1, OUTPUT);
pinMode(Finger1_2, OUTPUT);
pinMode(Finger1_3, OUTPUT);
pinMode(Finger1_4, OUTPUT);
pinMode(Finger2_1, OUTPUT);
pinMode(Finger2_2, OUTPUT);
pinMode(Finger2_3, OUTPUT);
pinMode(Finger2_4, OUTPUT);
pinMode(Finger3_1, OUTPUT);
pinMode(Finger3_2, OUTPUT);
pinMode(Finger3_3, OUTPUT);
pinMode(Finger3_4, OUTPUT);
}

void loop() {
  // put your main code here, to run repeatedly:

if (myFlexSensor.available() == true)
  {
    Serial.print(myFlexSensor.getX());
    Serial.println();
    command = myFlexSensor.getX();
  }
  delay(10);

//if (Serial.available()>0){
//int command = Serial.read();
Serial.println(command); 
if (command < 50 and command >= -50){
  Serial.println("You sent 2");
  Serial.println("Open Claw");
  analogWrite(Finger1_4, Active);
  analogWrite(Finger2_2, Active);
  analogWrite(Finger3_4, Active);
  Serial.println("Start");
  delay(TimeA);
  Serial.println("Stop");
  analogWrite(Finger1_4, 0);
  analogWrite(Finger2_2, 0);
  analogWrite(Finger3_4, 0);
}
else if (command >= 100 and command <= 200){
  Serial.println("You sent 3");
  Serial.println("Close Call");
  analogWrite(Finger1_2, Active);
  analogWrite(Finger2_3, Active);
  analogWrite(Finger3_2, Active);
  delay(TimeA);
  analogWrite(Finger1_2, 0);
  analogWrite(Finger2_3, 0);
  analogWrite(Finger3_2, 0);
}
else if (command > 200){
  Serial.println("You sent 4");
  Serial.println("Safety");
  analogWrite(Finger1_4, 0);
  analogWrite(Finger2_2, 0);
  analogWrite(Finger3_4, 0);
  analogWrite(Finger1_2, 0);
  analogWrite(Finger2_3, 0);
  analogWrite(Finger3_2, 0);
  delay(TimeA);
  analogWrite(Finger1_4, 0);
  analogWrite(Finger2_2, 0);
  analogWrite(Finger3_4, 0);
  analogWrite(Finger1_2, 0);
  analogWrite(Finger2_3, 0);
  analogWrite(Finger3_2, 0);
}
else if (command >= 50 and command < 100){
  Serial.println("You sent 4");
  Serial.println("Soft Claw Natural Off");
  analogWrite(Finger1_4, 0);
  analogWrite(Finger2_2, 0);
  analogWrite(Finger3_4, 0);
  analogWrite(Finger1_2, 0);
  analogWrite(Finger2_3, 0);
  analogWrite(Finger3_2, 0);
  delay(TimeA);
  analogWrite(Finger1_4, 0);
  analogWrite(Finger2_2, 0);
  analogWrite(Finger3_4, 0);
  analogWrite(Finger1_2, 0);
  analogWrite(Finger2_3, 0);
  analogWrite(Finger3_2, 0);
}
// else if (command == 54){
//  Serial.println("You sent 6");
//  Serial.println("Move Long Brac Up");
//  analogWrite(LBracU, dutyBL);
// delay(TimeA);
//  analogWrite(LBracU, 0);
//}
//else if (command == 55){
//  Serial.println("You sent 7");
//  Serial.println("Move Long Brac Down");
//  analogWrite(LBracD, dutyBL);
//  delay(TimeA);
//  analogWrite(LBracD, 0);
//}
else{
  Serial.println("None");
  analogWrite(Finger1_1, 0);
  analogWrite(Finger1_2, 0);
  analogWrite(Finger1_3, 0);
  analogWrite(Finger1_4, 0);
  analogWrite(Finger2_1, 0);
  analogWrite(Finger2_2, 0);
  analogWrite(Finger2_3, 0);
  analogWrite(Finger2_4, 0);
  analogWrite(Finger3_1, 0);
  analogWrite(Finger3_2, 0);
  analogWrite(Finger3_3, 0);
  analogWrite(Finger3_4, 0);
}
}
